...
CHOICES_COMPLIANCE = [("COVIDCompliant", "COVID-19 Compliant"), 
                      ("MOPDCompliant", "MOPD Compliant")]


CHOICES_PRICE = [("price_1", "$"), 
                 ("price_2", "$$"), 
                 ("price_3", "$$$"), 
                 ("price_4", "$$$$")]
...
def add_neighbourhood():
    for neighbourhood in CHOICES_NEIGHBOURHOOD:
        p = Preferences(preference_type="neighbourhood", value=neighbourhood[0], display_value=neighbourhood[1])
        p.save()


def add_category():
    for category in CHOICES_CATEGORY:
        p = Preferences(preference_type="category", value=category[0], display_value=category[1])
        p.save()


def add_rating():
    for rating in CHOICES_RATING:
        p = Preferences(preference_type="rating", value=rating[0], display_value=rating[1])
        p.save()


def add_compliance():
    for compliance in CHOICES_COMPLIANCE:
        p = Preferences(preference_type="compliance", value=compliance[0], display_value=compliance[1])
        p.save()
